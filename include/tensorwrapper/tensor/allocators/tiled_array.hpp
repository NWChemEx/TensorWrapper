...
public:
    ...
    explicit TiledArrayAllocator( ta::Storage storage = ta::Storage::Core,
      ta::Tiling tiling = ta::Tiling::OneBigTile, 
      ta::Distribution dist = ta::Distribution::Replicated,
      runtime_reference rt = TA::get_default_world()) : base_type(rt),
    ...
    bool operator==(const my_type& other) const {
        return storage_ == other.storage_ and 
               tiling_ == other.tiling_   and 
	       dist_ == other.dist_;
    }
