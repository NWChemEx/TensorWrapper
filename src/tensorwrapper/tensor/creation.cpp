...
ScalarTensorWrapper eigen_to_tensor_wrapper(const Eigen::MatrixXd& matrix,
    ...
    if(tiling.size() != 2)
        throw std::invalid_argument(
          "Eigen Conversion Requires Matrix (2D) Shape)");
    if(not tiling[0].size() or not tiling[1].size())
        throw std::invalid_argument(
          "Shape Tiling Cannot Be Empty in Eigen Conversion");
    if(not tiling[0].front() == 0 and not tiling[0].back() == matrix.rows())
        throw std::invalid_argument(
          "Shape Row Tiling and Matrix Row Count Incompatible");
    if(not tiling[1].front() == 0 and not tiling[1].back() == matrix.cols())
        throw std::invalid_argument(
          "Shape Col Tiling and Matrix Col Count Incompatible");
