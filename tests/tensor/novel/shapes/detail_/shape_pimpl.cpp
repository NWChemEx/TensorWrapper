...
TEST_CASE("ShapePIMPL<Tensor>") {
    ...
    pimpl_type defaulted;
    auto vov = testing::make_uniform_tot_shape<pimpl_type>(vector_extents, vector_extents);
    auto vom = testing::make_uniform_tot_shape<pimpl_type>(vector_extents, matrix_extents);
    auto vot = testing::make_uniform_tot_shape<pimpl_type>(vector_extents, tensor_extents);
    auto mov = testing::make_uniform_tot_shape<pimpl_type>(matrix_extents, vector_extents);
    auto mom = testing::make_uniform_tot_shape<pimpl_type>(matrix_extents, matrix_extents);
    auto mot = testing::make_uniform_tot_shape<pimpl_type>(matrix_extents, tensor_extents);
    auto tov = testing::make_uniform_tot_shape<pimpl_type>(tensor_extents, vector_extents);
    auto tom = testing::make_uniform_tot_shape<pimpl_type>(tensor_extents, matrix_extents);
    auto tot = testing::make_uniform_tot_shape<pimpl_type>(tensor_extents, tensor_extents);
    ...
    SECTION("CTors") {
	    ...
	    for( auto k = 0ul; k < 5; ++k ) {
            ...
            // Make sure object is forwarded correctly (i.e. no copy)
            auto pt = tensor_extents.data();
	    auto _dummy_map = testing::make_uniform_tot_map(tensor_extents,vector_extents);
            pimpl_type tensor2(std::move(tensor_extents),
                               std::move(_dummy_map));
            REQUIRE(tensor2.extents().data() == pt);
    ...
    SECTION("hash") {
        ...
        test_hash(vov, vector_extents, testing::make_uniform_tot_map(vector_extents, vector_extents));
        test_hash(vom, vector_extents, testing::make_uniform_tot_map(vector_extents, matrix_extents));
        test_hash(vot, vector_extents, testing::make_uniform_tot_map(vector_extents, tensor_extents));
        test_hash(mov, matrix_extents, testing::make_uniform_tot_map(matrix_extents, vector_extents));
        test_hash(mom, matrix_extents, testing::make_uniform_tot_map(matrix_extents, matrix_extents));
        test_hash(mot, matrix_extents, testing::make_uniform_tot_map(matrix_extents, tensor_extents));
        test_hash(tov, tensor_extents, testing::make_uniform_tot_map(tensor_extents, vector_extents));
        test_hash(tom, tensor_extents, testing::make_uniform_tot_map(tensor_extents, matrix_extents));
        test_hash(tot, tensor_extents, testing::make_uniform_tot_map(tensor_extents, tensor_extents));
        ...
        test_hash_false(vov, extents_type{5}, testing::make_uniform_tot_map(extents_type{5}, vector_extents ));
        test_hash_false(vov, vector_extents,  testing::make_uniform_tot_map(vector_extents,  extents_type{5}));
        test_hash_false(vov, extents_type{5}, testing::make_uniform_tot_map(extents_type{5}, extents_type{5}));
    }

    SECTION("Equality") {
        REQUIRE(vov == testing::make_uniform_tot_shape<pimpl_type>(vector_extents, vector_extents));
        REQUIRE(vom == testing::make_uniform_tot_shape<pimpl_type>(vector_extents, matrix_extents));
        REQUIRE(vot == testing::make_uniform_tot_shape<pimpl_type>(vector_extents, tensor_extents));
        REQUIRE(mov == testing::make_uniform_tot_shape<pimpl_type>(matrix_extents, vector_extents));
        REQUIRE(mom == testing::make_uniform_tot_shape<pimpl_type>(matrix_extents, matrix_extents));
        REQUIRE(mot == testing::make_uniform_tot_shape<pimpl_type>(matrix_extents, tensor_extents));
        REQUIRE(tov == testing::make_uniform_tot_shape<pimpl_type>(tensor_extents, vector_extents));
        REQUIRE(tom == testing::make_uniform_tot_shape<pimpl_type>(tensor_extents, matrix_extents));
        REQUIRE(tot == testing::make_uniform_tot_shape<pimpl_type>(tensor_extents, tensor_extents));
        ...
        REQUIRE_FALSE(vov == testing::make_uniform_tot_shape<pimpl_type>(extents_type{5}, extents_type{5}));
    }
}
