...
inline auto make_uniform_tot_map( const std::vector<size_t>& outer_extents, const std::vector<size_t>& inner_extents ) {
    ...
    size_t volume = 1; for(auto i : outer_extents) volume *= i;
...
template<typename RetT = 
  tensorwrapper::tensor::novel::Shape<tensorwrapper::tensor::field::Tensor>>
auto make_uniform_tot_shape( const std::vector<size_t>& outer_extents, const std::vector<size_t>& inner_extents ) {
    return RetT(outer_extents,make_uniform_tot_map(outer_extents,inner_extents));
}
