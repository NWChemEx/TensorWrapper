...
TEST_CASE("generate_eigen_system") {
    SECTION("shapes") {
        ...
        spec.seed             = 11;
        auto system           = generate_eigen_system(spec);
    ...
    SECTION("symmetry") {
        ...
        REQUIRE(approximately_equal(sym, tran, 1e-12));
    }
    ...
    SECTION("exact eigenpair") {
        ...
        REQUIRE(approximately_equal(av, scaled, 1e-12));
    }
    ...
    SECTION("deterministic for fixed seed") {
        ...
        spec.n       = 4;
        spec.seed    = 17;
        auto system1 = generate_eigen_system(spec);
        auto system2 = generate_eigen_system(spec);
    ...
    SECTION("degenerate") {
        ...
        auto evals = make_tensor({2}, std::vector<double>{1.0, 1.0});
        REQUIRE(approximately_equal(system.eigenvalues, evals));

        auto ident = make_tensor({2, 2}, std::vector<double>{1, 0, 0, 1});
        REQUIRE(approximately_equal(system.matrix, ident, 1e-12));
    }

    SECTION("invalid n throws") {
        SymmetricMatrixSpec spec;
        spec.n = 0;
        REQUIRE_THROWS_AS(generate_eigen_system(spec), std::invalid_argument);
    }
}
